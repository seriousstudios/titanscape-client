package com.jagex.actor.model;

import com.jagex.Configuration;
import com.jagex.actor.Renderable;
...
import com.jagex.net.requester.Provider;
...
public class Model extends Renderable {
    ...
    public void calculateVertexData(int value) {
        super.modelBaseY = 0;
        maxVertexDistanceXZPlane = 0;
        maximumYVertex = 0;
        ...
        if (value != 21073) {
            ...
            maxVertexDistanceXZPlane = diagonal3DAboveOrigin + (int) Math.sqrt(maxVertexDistanceXZPlane * maxVertexDistanceXZPlane + maximumYVertex * maximumYVertex);
            return;
        }
    }
...
/*	private void transformSkin(int i, int ai[], int j, int k, int l) {
        ...
        for (int i2 = 0; i2 < numTriangles; i2++) {
            if (triangleColours != null && face_alpha != null)
                if (triangleColours[i2] == 65535 // Most triangles
                        // || triangleColours[i2] == 0 //Black Triangles 633
                        // Models - Fixes Gwd walls & Black models
                        || triangleColours[i2] == 16705 // Nezzy Green
                    // Triangles//GWD White
                    // Triangles
                        )
                    face_alpha[i2] = 255;
            ...
            if (faceDrawType == null || (faceDrawType[i2] & 1) == 0) {
                ...
                faceHslA[i2] = method481(triangleColours[i2], l5, faceDrawType[i2]);
    ...
    private final void method484(int i) {
        ...
        if (faceDrawType == null)
            ...
            i1 = faceDrawType[i] & 3;
        if (i1 == 0) {
            Rasterizer3D.drawShadedTriangle(projected_vertex_y[j], projected_vertex_y[k], projected_vertex_y[l], projected_vertex_x[j], projected_vertex_x[k], projected_vertex_x[l], faceHslA[i], faceHslB[i], faceHslC[i]);
            return;
        }
        if (i1 == 1) {
        ...
        if ((j3 - j4) * (k7 - j7) - (i7 - j7) * (j5 - j4) > 0) {
            ...
            if (l == 3) {
                ...
                if (l7 == 0) {
                    ...
                    Rasterizer3D.drawTexturedTriangle(i7, j7, k7, j3, j4, j5, anIntArray1680[0], anIntArray1680[1], anIntArray1680[2], anIntArray1668[k9], anIntArray1668[k10], anIntArray1668[k11], camera_vertex_y[k9], camera_vertex_y[k10], camera_vertex_y[k11], camera_vertex_x[k9], camera_vertex_x[k10], camera_vertex_x[k11], triangleColours[i]);
                } else if (l7 == 3) {
                    int k8 = faceDrawType[i] >> 2;
                    int l9 = textures_face_a[k8];
                    int l10 = textures_face_b[k8];
                    int l11 = textures_face_c[k8];
                    Rasterizer3D.drawTexturedTriangle(i7, j7, k7, j3, j4, j5, faceHslA[i], faceHslA[i], faceHslA[i], anIntArray1668[l9], anIntArray1668[l10], anIntArray1668[l11], camera_vertex_y[l9], camera_vertex_y[l10], camera_vertex_y[l11], camera_vertex_x[l9], camera_vertex_x[l10], camera_vertex_x[l11], triangleColours[i]);
                }
            }
            if (l == 4) {
                if (j3 < 0 || j4 < 0 || j5 < 0 || j3 > Rasterizer2D.centerX
