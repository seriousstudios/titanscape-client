        ...
        for (int y = 0; y < Rasterizer2D.bottomY; y++) {
            for (int x = 0; x < Rasterizer2D.centerX; x++) {
                if (Rasterizer2D.depthBuffer[pos] >= fogEnd) {
                    Rasterizer2D.rasterPixels[pos] = setColor;
                    ...
                    alpha = 256 - alpha;
                    dst = Rasterizer2D.rasterPixels[pos];
                    dst = ((dst & 0xff00ff) * alpha >> 8 & 0xff00ff) + ((dst & 0xff00) * alpha >> 8 & 0xff00);
                    Rasterizer2D.rasterPixels[pos] = src + dst;
                }
                pos++;
            }
            pos += Rasterizer2D.rasterWidth - Rasterizer2D.centerX;
        }
    }
