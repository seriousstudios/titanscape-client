...
public final class Sprite extends Rasterizer2D {
    ...
    /**
     * The identifier or index of this sprite (used for the sprite packer)
     */
    private int id;

    /**
     * The name of this sprite (optionally for the sprite packer)
     */
    private String name;

    /**
     * The pixels that make up this sprite in ARGB format.
     */
    private int pixels[];

    /**
     * The actual width of this sprite.
     */
    private int width;

    /**
     * The actual height of this sprite.
     */
    private int height;

    /**
     * The horizontal offset used for display a sprite on an interface.
     */
    private int offsetX;

    /**
     * The vertical offset used for displaying a sprite on an interface.
     */
    private int offsetY;

    /**
     * The largest width of the sprite within the same image archive.
     */
    private int largestWidth;

    /**
     * The largest height of the sprite within the same image archive.
     */
    ...
    public Sprite(FileArchive archive, String name, int spriteId) {
        ByteBuffer dataBuffer = ByteBuffer.wrap(archive.readFile(name + ".dat"));
        ByteBuffer indexBuffer = ByteBuffer.wrap(archive.readFile("index.dat"));

        // position of the image archive within the archive
        indexBuffer.position(dataBuffer.getShort() & 0xFFFF);

        // the image with the largest width
        largestWidth = indexBuffer.getShort() & 0xFFFF;

        // the image width the largest height
        largestHeight = indexBuffer.getShort() & 0xFFFF;

        // the number of colors that are used in this image archive
        int colors = indexBuffer.get() & 0xFF;
        ...
        for (int i = 0; i < colors - 1; i++) {
            ...
            palette[i + 1] = ByteBufferUtils.readU24Int(indexBuffer);

            // + 1 because index = 0 is for transparency, = 1 because solid black was used originally used for transparency
            if (palette[i + 1] == 0) {
                palette[i + 1] = 1;
            }
        }

        for (int index = 0; index < spriteId; index++) {

            // we want to skip the next 2 bytes (which is offsetX and offsetY to read the width and height of the image)
            indexBuffer.position(indexBuffer.position() + 2);

            // the width of the sprite at this position in the image archive
            int width = indexBuffer.getShort() & 0xFFFF;

            // the height of the sprite at this position in the image archive
            int height = indexBuffer.getShort() & 0xFFFF;

            // skip this entire sprite
            dataBuffer.position(dataBuffer.position() + (width * height));

            // lastly skip the type of image which has a size of 1
            indexBuffer.position(indexBuffer.position() + 1);
        }

        // used for interfaces
        offsetX = indexBuffer.get() & 0xFF;
        offsetY = indexBuffer.get() & 0xFF;

        // actual width of this sprite
        width = indexBuffer.getShort() & 0xFFFF;

        // actual height of this sprite
        height = indexBuffer.getShort() & 0xFFFF;

        // theres 2 types (0 or 1, 0 means the position is read horizontally, 1 means vertically)
        int type = indexBuffer.get() & 0xFF;

        pixels = new int[width * height];

        if (type == 0) { // read horizontally
            for (int pixel = 0; pixel < pixels.length; pixel++) {
                pixels[pixel] = palette[dataBuffer.get() & 0xFF];
            }

        } else if (type == 1) { // read vertically
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
                    pixels[x + y * width] = palette[dataBuffer.get() & 0xFF];
                }
            }

        }

        // lastly after the image is fully read transparency is set
        setTransparency(255, 0, 255);
    }

    public Sprite(int i, int j) {
        pixels = new int[i * j];
        width = largestWidth = i;
        height = largestHeight = j;
        offsetX = offsetY = 0;
    }
    ...
    public void draw24BitSprite(int x, int y) {
        ...
        int destOffset = x + y * Rasterizer2D.rasterWidth;
        int srcOffset = 0;
        int height = this.height;
        int width = this.width;
        int destStep = Rasterizer2D.rasterWidth - width;
        int srcStep = 0;
        if (y < Rasterizer2D.topY) {
            ...
            destOffset += trimHeight * Rasterizer2D.rasterWidth;
        }
        ...
        if (!((width <= 0) || (height <= 0))) {
            set24BitPixels(width, height, Rasterizer2D.rasterPixels, pixels, alpha, destOffset, srcOffset, destStep, srcStep);
        }
    }
    ...
    public void drawTransparentSprite(int x, int y, int opacity) {
        ...
        x += offsetX;
        y += offsetY;
        int i1 = x + y * Rasterizer2D.rasterWidth;
        int j1 = 0;
        int k1 = height;
        int l1 = width;
        int i2 = Rasterizer2D.rasterWidth - l1;
        int j2 = 0;
        if (y < Rasterizer2D.topY) {
            ...
            y = Rasterizer2D.topY;
            j1 += k2 * l1;
            i1 += k2 * Rasterizer2D.rasterWidth;
        }
        ...
        if (!(l1 <= 0 || k1 <= 0)) {
            method351(j1, l1, Rasterizer2D.rasterPixels, pixels, j2, k1, i2, k, i1);
        }
    }
    ...
    public void init() {
        Rasterizer2D.initDrawingArea(height, width, pixels, null);
    }
    ...
    public void method345() {
        int ai[] = new int[largestWidth * largestHeight];
        for (int j = 0; j < height; j++) {
            System.arraycopy(pixels, j * width, ai, j + offsetY * largestWidth + offsetX, width);
        }
    ...
    public void method346(int x, int y) {
        x += offsetX;
        y += offsetY;
        int l = x + y * Rasterizer2D.rasterWidth;
        int i1 = 0;
        int height = this.height;
        int width = this.width;
        int l1 = Rasterizer2D.rasterWidth - width;
        int i2 = 0;
        if (y < Rasterizer2D.topY) {
        ...
        if (width <= 0 || height <= 0) {
            ...
            method347(l, width, height, i2, i1, l1, pixels, Rasterizer2D.rasterPixels);
        }
    }
    ...
    public void drawSprite1(int i, int j, int k) {
        i += offsetX;
        j += offsetY;
        int i1 = i + j * Rasterizer2D.rasterWidth;
        int j1 = 0;
        int k1 = height;
        int l1 = width;
        int i2 = Rasterizer2D.rasterWidth - l1;
    ...
    public void drawSprite(int x, int y) {
        x += offsetX;
        y += offsetY;
        int rasterClip = x + y * Rasterizer2D.rasterWidth;
        int imageClip = 0;
        int height = this.height;
        int width = this.width;
        int rasterOffset = Rasterizer2D.rasterWidth - width;
        int imageOffset = 0;
        if (y < Rasterizer2D.topY) {
            ...
            y = Rasterizer2D.topY;
            imageClip += dy * width;
            rasterClip += dy * Rasterizer2D.rasterWidth;
        }
        if (y + height > Rasterizer2D.bottomY)
            height -= (y + height) - Rasterizer2D.bottomY;
        if (x < Rasterizer2D.leftX) {
        ...
        if (!(width <= 0 || height <= 0)) {
            method349(Rasterizer2D.rasterPixels, pixels, imageClip, rasterClip, width, height, rasterOffset, imageOffset);
        }
    }
    ...
    public void drawSprite(int i, int k, int color) {
        ...
        i += offsetX;
        k += offsetY;
        int l = i + k * Rasterizer2D.rasterWidth;
        int i1 = 0;
        int j1 = tempHeight;
        int k1 = tempWidth;
        int l1 = Rasterizer2D.rasterWidth - k1;
        int i2 = 0;
        ...
        if (!(k1 <= 0 || j1 <= 0)) {
            method349(Rasterizer2D.rasterPixels, tempArray, i1, l, k1, j1, l1, i2);
        }
    }
    ...
    public void rotate(int i, int j, int ai[], int k, int ai1[], int i1, int j1, int k1, int l1, int i2) {
        try {
            ...
            int l3 = k1 + j1 * Rasterizer2D.rasterWidth;
            for (j1 = 0; j1 < i; j1++) {
                int i4 = ai1[j1];
                int j4 = l3 + i4;
                int k4 = j3 + i3 * i4;
                ...
                for (k1 = -ai[j1]; k1 < 0; k1++) {
                    ...
                    int x2 = x1 + 1;
                    int y2 = y1 + 1;
                    int c1 = pixels[x1 + y1 * width];
                    int c2 = pixels[x2 + y1 * width];
                    int c3 = pixels[x1 + y2 * width];
                    int c4 = pixels[x2 + y2 * width];
                    int u1 = (k4 >> 8) - (x1 << 8);
                    int v1 = (l4 >> 8) - (y1 << 8);
                    int u2 = (x2 << 8) - (k4 >> 8);
                    int v2 = (y2 << 8) - (l4 >> 8);
                    ...
                    Rasterizer2D.rasterPixels[j4++] = r | g | b;
                    k4 += i3;
                    l4 -= l2;
                }
    ...
    public void method353(int i, double d, int l1) {
        ...
        try {
            ...
            int k3 = l1 + i * Rasterizer2D.rasterWidth;
            for (i = 0; i < k1; i++) {
                int l3 = k3;
                int i4 = i3;
                int j4 = j3;
                for (l1 = -k; l1 < 0; l1++) {
                    ...
                    if (k4 != 0)
                        Rasterizer2D.rasterPixels[l3++] = k4;
                    else
                        l3++;
                    i4 += l2;
                    j4 -= k2;
    ...
    public void method354(IndexedImage background, int i, int j) {
        j += offsetX;
        i += offsetY;
        int k = j + i * Rasterizer2D.rasterWidth;
        int l = 0;
        int i1 = height;
        int j1 = width;
        int k1 = Rasterizer2D.rasterWidth - j1;
        int l1 = 0;
        ...
        if (!(j1 <= 0 || i1 <= 0)) {
            method355(pixels, j1, background.palettePixels, i1, Rasterizer2D.rasterPixels, 0, k1, k, l1, l);
        }
    }
    ...
    public void drawARGBSprite(int xPos, int yPos, int alpha) {
        ...
        xPos += offsetX;
        yPos += offsetY;
        int i1 = xPos + yPos * Rasterizer2D.rasterWidth;
        int j1 = 0;
        int spriteHeight = height;
        int spriteWidth = width;
        int i2 = Rasterizer2D.rasterWidth - spriteWidth;
        int j2 = 0;
        ...
        if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
            renderARGBPixels(spriteWidth, spriteHeight, pixels, Rasterizer2D.rasterPixels, i1, alphaValue, j1, j2, i2);
        }
    }
