...
import com.jagex.Configuration;
...
import com.jagex.collection.ReferenceCache;
...
public final class Widget {
    ...
    public static void load(FileArchive interfaces, GameFont textDrawingAreas[], FileArchive graphics) {
        ...
        while (buffer.currentPosition < buffer.payload.length) {
            ...
            if (widget.type == TYPE_MODEL) {
                ...
                if (content != 0) {
                    widget.modelType = 1;
                    widget.modelFileId = (content - 1 << 8) + buffer.readUnsignedByte();
                }
                content = buffer.readUnsignedByte();
                if (content != 0)
                ...
                widget.modelRotationY = buffer.readUShort();
                widget.modelRotationX = buffer.readUShort();
            }
    ...
    public static void addSprite(int interfaceId, int disabledSpriteId, int enabledSpriteId) {
        ...
        rsinterface.disabledSprite = Client.spriteCache.lookup(disabledSpriteId);
        rsinterface.enabledSprite = Client.spriteCache.lookup(enabledSpriteId);
    }
    ...
    public static void addConfigButton(int ID, int pID, int disabledSpriteId, int enabledSpriteId, int width,
    ...
    public static Model getModel(int type, int fileId) {

        Model model = (Model) models.get((type << 16) + fileId);
        ...
        if (type == 1) {
            model = Model.getModel(fileId);
        }

        if (type == 2) {
            model = NpcDefinition.lookup(fileId).model();
        }
        ...
        if (type == 4) {
            model = ItemDefinition.lookup(fileId).getUnshadedModel(50);
        }
        ...
        if (model != null) {
            models.put(model, (type << 16) + fileId);
        }
    ...
    public Model getModel(int id, int frame, boolean params) {
        Model model;
        if (params)
            model = getModel(modelType, modelFileId);
        else
            model = getModel(defaultMediaType, defaultMedia);
        if (model == null)
            return null;
        if (frame == -1 && id == -1 && model.triangleColours == null)
            return model;
        Model model_1 = new Model(true, Frame.noAnimationInProgress(frame) & Frame.noAnimationInProgress(id), false, model);
        if (frame != -1 || id != -1)
            model_1.skin();
        if (frame != -1)
            model_1.applyTransform(frame);
        if (id != -1)
            model_1.applyTransform(id);
    ...
    public int type;
    public int x;
    public static final ReferenceCache models = new ReferenceCache(30);
    public int anInt265;
    ...
    public static void addLunarSprite(int i, int spriteId) {
        ...
        RSInterface.disabledSprite = Client.spriteCache.lookup(spriteId);
    ...
    public static void drawRune(int i, int id, String runeName) {
        ...
        RSInterface.opacity = 0;
        RSInterface.hoverType = 52;
        RSInterface.disabledSprite = Client.spriteCache.lookup(id + 185);
        RSInterface.width = 500;
        RSInterface.height = 500;
    }
    ...
    public static void homeTeleport() {
        ...
        RSInterface.disabledSprite = Client.spriteCache.lookup(201);
        RSInterface.width = 20;
        RSInterface.height = 20;
    ...
    public static void addLunar2RunesSmallBox(int ID, int r1, int r2, int ra1, int ra2, int rune1, int lvl, String name,
        ...
        rsInterface.enabledSprite = Client.spriteCache.lookup(sid + 147);
        rsInterface.disabledSprite = Client.spriteCache.lookup(sid + 108);
    ...
    public static void addButton(int i, int spriteId, int W, int H, String S, int AT) {
        ...
        RSInterface.hoverType = 52;
        RSInterface.disabledSprite = Client.spriteCache.lookup(spriteId);
        RSInterface.enabledSprite = Client.spriteCache.lookup(spriteId);
        RSInterface.width = W;
        RSInterface.height = H;
        RSInterface.tooltip = S;
    }

    public static void addConfigHover(int interfaceID, int actionType, int hoverid, int disabledSpriteId,
                                      int enabledSpriteId, int Width, int Height, int configFrame, int configId, String Tooltip, int hoverId2,
        ...
        hover.disabledSprite = Client.spriteCache.lookup(disabledSpriteId);
        hover.enabledSprite = Client.spriteCache.lookup(enabledSpriteId);
        hover.width = Width;
    ...
    public static void addSprites(int ID, int disabledSpriteId, int enabledSpriteId, String name, int configId,
                                  int configFrame) {
        Widget Tab = addTabInterface(ID);
        Tab.id = ID;
        Tab.parent = ID;
