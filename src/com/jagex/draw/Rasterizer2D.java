...
public class Rasterizer2D extends Cacheable {
    ...
    public static void initDrawingArea(int height, int width, int pixels[], float depth[]) {
        depthBuffer = depth;
        Rasterizer2D.rasterPixels = pixels;
        Rasterizer2D.rasterWidth = width;
        Rasterizer2D.rasterHeight = height;
    ...
    public static void drawTransparentGradientBox(int leftX, int topY, int width, int height, int topColour, int bottomColour, int opacity) {
        ...
        if (topY + height > bottomY)
            height = bottomY - topY;
        int leftOver = Rasterizer2D.rasterWidth - width;
        int transparency = 256 - opacity;
        int pixelIndex = leftX + topY * Rasterizer2D.rasterWidth;
    ...
    public static void setDrawingArea(int bottomY, int leftX, int rightX, int topY) {
        ...
        if (rightX > rasterWidth) {
            rightX = rasterWidth;
        }
        if (bottomY > rasterHeight) {
            bottomY = rasterHeight;
        }
    ...
    public static void clear() {
        int i = rasterWidth * rasterHeight;
        for (int j = 0; j < i; j++) {
            rasterPixels[j] = 0;
    ...
    public static void drawBox(int leftX, int topY, int width, int height, int rgbColour) {
        ...
        for (int rowIndex = 0; rowIndex < height; rowIndex++) {
            for (int columnIndex = 0; columnIndex < width; columnIndex++)
                rasterPixels[pixelIndex++] = rgbColour;
            pixelIndex += leftOver;
        }
    }
    ...
    public static void drawTransparentBox(int leftX, int topY, int width, int height, int rgbColour, int opacity) {
        ...
        for (int rowIndex = 0; rowIndex < height; rowIndex++) {
            for (int columnIndex = 0; columnIndex < width; columnIndex++) {
                int otherRed = (rasterPixels[pixelIndex] >> 16 & 0xff) * transparency;
                int otherGreen = (rasterPixels[pixelIndex] >> 8 & 0xff) * transparency;
                int otherBlue = (rasterPixels[pixelIndex] & 0xff) * transparency;
                int transparentColour = ((red + otherRed >> 8) << 16) + ((green + otherGreen >> 8) << 8) + (blue + otherBlue >> 8);
                rasterPixels[pixelIndex++] = transparentColour;
            }
            pixelIndex += leftOver;
        }
    }
    ...
    public static void drawHorizontalLine(int xPosition, int yPosition, int width, int rgbColour) {
        ...
        int pixelIndex = xPosition + yPosition * Rasterizer2D.rasterWidth;
        for (int i = 0; i < width; i++)
            rasterPixels[pixelIndex + i] = rgbColour;
    }
    ...
    public static void drawVerticalLine(int xPosition, int yPosition, int height, int rgbColour) {
        ...
        int pixelIndex = xPosition + yPosition * rasterWidth;
        for (int rowIndex = 0; rowIndex < height; rowIndex++)
            rasterPixels[pixelIndex + rowIndex * rasterWidth] = rgbColour;
    }
    ...
    public static void drawTransparentHorizontalLine(int xPosition, int yPosition, int width, int rgbColour, int opacity) {
        ...
        int pixelIndex = xPosition + yPosition * Rasterizer2D.rasterWidth;
        for (int i = 0; i < width; i++) {
            final int otherRed = (rasterPixels[pixelIndex] >> 16 & 0xff) * transparency;
            final int otherGreen = (rasterPixels[pixelIndex] >> 8 & 0xff) * transparency;
            final int otherBlue = (rasterPixels[pixelIndex] & 0xff) * transparency;
